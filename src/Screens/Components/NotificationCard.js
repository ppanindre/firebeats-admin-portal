...
const NotificationCard = ({ userId, notificationId, data, onApprove, onReject, isApproved  }) => {
  ...
  const fetchActivity = async (date) => {
    ...
    const startIndexTime = moment(data?.heartRates[0].timeStamp, "HH:mm:ss")
      .diff(moment().startOf("d"), "minute")
    const endIndexTime = moment(data?.heartRates[data?.heartRates.length - 1]
      .timeStamp, "HH:mm:ss")
      .diff(moment().startOf("d"), "minute")
    ...
    let adjustedActivityData = (fetchedActivityData ?? [])
      ...
      .map((data, index) => {
        return {
          x: moment().startOf("d").add(adjustedStartIndex + index, "minute")
            .toDate().getTime(),
          y: data
        }
      });
    
      if (adjustedActivityData.length == 0 ||
        (adjustedActivityData.length != 0 &&
          adjustedActivityData.reduce((a, b) => a + parseInt(b.y), 0
          ) == 0)) {
    ...
    <div key={notificationId} style={cardStyle}>
...

      {
        data.type === "SLEEP" ? 
          ...
          :
          activityData?.length > 0 ?   <ActivityLineChart data={activityData} useTimestamp={true}
          loading={false} isNotification={true} />
      ...
      {!isApproved && (
        <div style={{ marginTop: '8px' }}>
          ...
          <button onClick={() => onReject(userId, notificationId, data)}>Reject</button>
        </div>
      )}
    </div>
  );
